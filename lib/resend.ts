...
const FROM_EMAIL = 'BullVision <info@thebullocracy.com>';
...
<body>
  <div class="wrapper">
    <div class="container">
      ...
      <div class="header">
        <h1>Bull<span>Vision</span></h1>
      </div>
      <div class="content">
        ${content}
      </div>
      <div class="footer">
        <p>&copy; ${new Date().getFullYear()} BullVision. Todos os direitos reservados.</p>
        <p><a href="https://thebullvision.com">Visitar a Plataforma</a></p>
      </div>
    </div>
  </div>
</body>
...
/**
 * Envia o email de Confirmação de conta (com a marca BullVision).
...
export const sendConfirmationEmail = async (email: string, name: string, confirmLink: string) => {
  ...
  return await resend.emails.send({
    from: FROM_EMAIL,
    to: [email],
    subject: 'Confirma a tua conta — BullVision',
    text: `Olá ${name},\n\nFalta só um passo para ativares a tua conta na BullVision.\nConfirma o teu email abrindo este link:\n\n${confirmLink}\n\nSe não foste tu a criar esta conta, ignora este email.`,
    html: getEmailTemplate(`
      <h2>Olá ${name}, confirma a tua conta</h2>
      <p>Falta só um passo para começares a usar a BullVision. Clica no botão abaixo para confirmar o teu email e ativar a conta.</p>
...
export const sendUpgradeToProEmail = async (email: string, name: string) => {
  ...
  return await resend.emails.send({
    ...
    html: getEmailTemplate(`
      ...
      <div style="text-align: center;">
        <a href="https://thebullvision.com/dashboard" class="btn">Explorar Funcionalidades PRO</a>
      </div>
    `),
  });
};
...
export const sendTrialConfirmationEmail = async (email: string, name: string) => {
  ...
  return await resend.emails.send({
    ...
    html: getEmailTemplate(`
      ...
      <div style="text-align: center;">
        <a href="https://thebullvision.com/dashboard" class="btn">Aproveitar o Trial</a>
      </div>
    `),
  });
};
...
export const sendPasswordResetEmail = async (email: string, resetLink: string) => {
  ...
  return await resend.emails.send({
    from: FROM_EMAIL,
    to: [email],
    subject: 'Recuperação de Password - BullVision',
    ...
    html: getEmailTemplate(`
      ...
      <p>Recebemos um pedido para repor a password da tua conta na BullVision.</p>
