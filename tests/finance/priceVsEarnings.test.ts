...
describe("buildPriceVsEarnings", () => {
  it("indexa as duas séries a 100 na mesma data-base", () => {
    const r = buildPriceVsEarnings(series(20, i => 100 + i, i => 2 + i * 0.1), null)
  ...
  it("preço a duplicar e lucro a triplicar dão índices 200 e 300 no fim", () => {
    const r = buildPriceVsEarnings(series(21, i => 100 * (1 + i / 20), i => 2 * (1 + (2 * i) / 20)), null)
  ...
  it("o gap de CAGR identifica expansão de múltiplo", () => {
    ...
    const r = buildPriceVsEarnings(
      series(q, i => 100 * Math.pow(4, i / (q - 1)), i => 2 * Math.pow(2, i / (q - 1))),
      null,
    )
  ...
  it("ancora no primeiro trimestre lucrativo quando houve prejuízos antes", () => {
    ...
    const r = buildPriceVsEarnings(
      series(24, i => 50 + i, i => (i < 8 ? -1 + i * 0.05 : 0.5 * (i - 7))),
      null,
    )
    ...
    expect(r.data[0].earningsIdx).toBe(100)
    expect(r.data.every(d => d.earnings > 0)).toBe(true)
  })

  it("devolve vazio quando a empresa nunca teve lucro na janela", () => {
    const r = buildPriceVsEarnings(series(20, i => 50 + i, () => -2), null)
    expect(r.data).toEqual([])
    expect(r.baseDate).toBeNull()
  })

  it("devolve vazio quando só o último ponto é lucrativo (nada para comparar)", () => {
    const r = buildPriceVsEarnings(series(12, i => 50 + i, i => (i === 11 ? 1 : -1)), null)
    expect(r.data).toEqual([])
  })

  it("ignora pontos sem lucro TTM ou com preço inválido", () => {
    const rows = series(12, i => 100 + i, i => 2 + i * 0.05)
    rows[3].epsTtm = null
    rows[5].price = 0
    rows[7].epsTtm = Number.NaN
  ...
  it("nunca devolve NaN no CAGR quando o lucro final é negativo", () => {
    ...
    const r = buildPriceVsEarnings(series(21, i => 100 + i, i => 2 - i * 0.2), null)
  ...
  it("sem 1 ano completo não calcula CAGR, só variação total", () => {
    const r = buildPriceVsEarnings(series(3, i => 100 + i, i => 2 + i * 0.05), null)
  ...
  it("a janela em anos corta a série a contar do último ponto", () => {
    const all = buildPriceVsEarnings(series(41, i => 100 + i, i => 2 + i * 0.05), null)
    const fiveY = buildPriceVsEarnings(series(41, i => 100 + i, i => 2 + i * 0.05), 5)
  ...
  it("faz downsample mas preserva o primeiro e o último ponto", () => {
    const rows = series(1_000, i => 100 + i, i => 2 + i * 0.05)
  ...
  it("sugere escala log quando uma das séries se multiplica muito", () => {
    ...
    const big = buildPriceVsEarnings(series(21, i => 100 + i, i => 0.05 * Math.pow(100, i / 20)), null)
    expect(big.logAvailable).toBe(true)
    expect(big.logSuggested).toBe(true)

    const calm = buildPriceVsEarnings(series(21, i => 100 + i, i => 2 + i * 0.1), null)
  ...
  it("uma série demasiado curta devolve vazio em vez de explodir", () => {
    ...
    expect(buildPriceVsEarnings(series(1, () => 100, () => 2), null).data).toEqual([])
  })
})
